...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.kidscademy.quiz.instruments"
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assemble && \
...
$(ESPRESSO_TESTS) : app-androidTest.apk app-instrumented.apk
	...
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.kidscademy.quiz.instruments))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.kidscademy.quiz.instruments; \
	else \
