...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.kidscademy.quiz.instruments  -Dabc.instrument.multithreaded "

INSTRUMENTATION_OPTS=" \
--skip-class=com.kidscademy.quiz.instruments.R \
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.kidscademy.quiz.instruments$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.kidscademy.quiz.instruments.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.kidscademy.quiz.instruments > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.kidscademy.quiz.instruments.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.kidscademy.quiz.instruments > /dev/null; \
	fi)
...
app-original.apk :
	...
	$(GW) -PjacocoEnabled=false assemble </dev/null && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	@export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true clean assemble assembleAndroidTest </dev/null && \
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.kidscademy.quiz.instruments.test/android.support.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.kidscademy.quiz.instruments ./traces/$(TEST_NAME) force-clean
